...
def decide(sig: dict, prev: dict, first_to: int, stall_to: int,
    ...
    **核心规则：资源占用只有在「资源画像符合该 workload 的预期」时才算进展证据。**
    （2026-08-15 实测修正，见 evidence/measure-06）

    信号分两类，但界线不在"是不是资源"，而在"这个观测值配不配得上叫进展"：

      · 日志进度行、产物文件 —— 无条件的进展证据
      · 资源占用 —— **有条件**的进展证据，条件是画像相符：
          纯 CPU workload（ROMS/WRF/MITgcm）：CPU 忙 = 画像相符 → 算进展
          GPU workload：GPU 忙 = 画像相符 → 算进展
          GPU workload：CPU 打满而 GPU 闲置 = **画像不符** → 不算进展，
                        而且是退化路径的正面指征（degradation_suspected）

    原实现把 CPU 占用当作与日志、产物完全对等的一票，不看画像。于是 GPU 训练
    掉进退化路径时（日志静默、无新产物、CPU 100%、GPU 0%）会被投成 RUNNING ——
    恰好漏报了本项目的头号故障。

    注意这条规则的代价：一个**在 GPU 上空转**的作业（GPU 忙但没有产出）
    当前判据覆盖不到 —— 因为 GPU 忙符合画像。这是已知边界，写在 SKILL.md 的局限里。
