...
## 8. 审批答复缺少幂等保护，重复答复可能导致重复执行

**严重程度：中（安全）**

### 复现

Worker 发出人工审批请求后，人的答复是**自由文本 + mention**。
网络抖动或客户端重发时，同一条答复可能到达多次（我们实测中因断线重连，
一条「拒绝」连发了 3 次），Worker 侧没有任何请求 ID 或消费标记来保证幂等。

我们这次的目标 Agent 靠模型自己识别出"这是重复消息"而未重复执行，
但这是**模型行为，不是机制保证**。

### 影响

若被重复批准的动作不幂等（例如"重启作业"），重复执行会造成真实损害。
同时，答复必须携带 `m.mentions` 才会被接收（见本文第 5 条），
两者叠加的后果是：**漏收与重收同时可能发生，且都没有提示**。

### 建议

框架层提供带 ID 的审批原语：请求方生成 `request_id`，
答复形如 `!approve <request_id>` / `!reject <request_id>`，
同一 ID 只能被消费一次，重复答复返回"该请求已处理"。

---

## 提交计划

上述 1–5、7、8 条向 AgentTeams 仓库提 issue，第 1 条因涉及凭证泄露，
