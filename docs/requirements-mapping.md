...
**已跑过能力评估的 Skill：3 个**（`progress-probe`、`config-precheck`、`safe-kill`），其余 5 个只写了评估方法。

> `safe-kill` 的评估是**对抗测试**而非统计指标 —— 它的 SKILL.md 里写着
> 「准确率 99% 对这个 Skill 是失败，剩下的 1% 就是别人的 128 核作业」。
> 20 项对抗用例已实现并随 `./verify.sh` 每次必跑，全部通过。
> 最关键的一条：**跨用户同名进程被明确拒绝，理由为属主不符** ——
> 而在 [case-02](../evidence/case-02-kill-by-name-is-broken.md) 那次真实事故里，
> 他人的 128 核作业是靠跨用户 kill 的 EPERM 静默失败才幸存的。
...
| TTFT | ❌ | 未采。框架不记录，且非流式调用拿不到，需改为流式才能测 |
| **Token 消耗** | ✅ | **已实测**（[measure-12](../evidence/measure-12-token-cost.md)）：单轮 1163+392=1555 tokens（推理占 244）；5 轮工具往返累计 9908，prompt 每轮增约 190。框架本身不记账，本数字为我们经网关实测所得 |
| Tool 成功率 | ✅ | **执行层 95.7%**（23 次调用，1 次被沙箱拦截、0 次报错）。唯一非成功是安全策略主动阻断 rm，属护栏正常工作。另有决策层 56/56 allow —— 两者含义不同，都给出 |
...
TTFT 未采；无人工侧成本基线，因此答不了「省了多少钱」；可观测未做 OTel 语义映射，也未接时序/向量后端。
