...
核心规则一句话：**资源占用只有在「资源画像符合该 workload 的预期」时才算进展证据。**

| 信号 | 采集方式 | 算不算进展 |
|---|---|---|
| A · 日志 | 新增片段里匹配到 adapter 的进度行 | **无条件算** |
| B · 输出文件 | 主输出文件的 `mtime` 或 `size` 变化 | **无条件算** |
| C · CPU | 作业进程的 CPU% | **有条件**：画像相符才算 |
| D · GPU | `nvidia-smi` 利用率（adapter 声明 `gpu_signal.required` 时才采） | **有条件**：同上 |

画像怎么算相符：

| workload | 相符（算进展） | 不符（不算，且可疑） |
|---|---|---|
| 纯 CPU（ROMS/WRF/MITgcm） | CPU 忙 | —— |
| GPU 训练 | GPU 忙 | **CPU 打满 + GPU 闲置** → 置 `degradation_suspected` |

这条规则是被实测推翻原设计后改的：原来 CPU 与日志、产物完全对等，
于是 GPU 退化路径上 CPU 那一票把状态硬拉成 RUNNING，**全程漏报**。
...
- **GPU 上空转覆盖不到**：一个 GPU 忙但没有任何产出的作业（比如在 GPU 上死循环），
  因为"GPU 忙"符合画像，会被判为 RUNNING。这是"画像相符即算进展"这条规则的直接代价，
  已知且未解决。要覆盖它需要更细的信号（如 SM 占用率与显存读写量的比值）。
